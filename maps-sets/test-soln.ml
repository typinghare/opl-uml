 ...
 *)
let test4 = try VarMap.find "x" test3
            with Not_found -> 0

(* lookup z from the map returned in test3;
   return 0 if not found
 *)
let test4b = try VarMap.find "z" test3
            with Not_found -> 0
...
let test8 = try VarSet.find "x" test7
with Not_found -> "Not found"

(* lookup z from the set returned in test7;
   return string "Not found" if there is no such element *)
let test9 = try VarSet.find "z" test7
with Not_found -> "Not found"
...
(*  Return a set that is union of test7 and { "z" }
  *)
let test12 = VarSet.union test7 (VarSet.add "z" VarSet.empty)
